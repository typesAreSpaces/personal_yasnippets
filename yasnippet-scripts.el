...
(defun yasnippet/input (input)
  (let* ((args (split-string input "," t "[[:space:]]*")))
    (pcase (length args)
      (3 (apply #'yasnippet/input_aux args))
      (2 (yasnippet/input_aux
          (nth 0 args)
          (nth 1 args)
          "section"))
      (1 (yasnippet/input_aux
          (car args)
          ""
          ""))
      (_ (yasnippet/input_aux
          "_new_file_"
          "new_title"
          "")))))

(defun yasnippet/input_aux (name title header)
  (let ((file-name (concat name ".tex")))
